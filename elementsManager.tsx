...
export default function elementsManager(element: PropsManager, index: number, index2: number) {
    ...
    switch (data.type)
    {
        ...
        case "table":
            return (<Table key={"section_"+index+"_table_"+index2} array={data.array} side={data.side} gapInside={data.gapInside} gapOutside={data.gapOutside} background={data.background}/>)
        case "downTransition":
