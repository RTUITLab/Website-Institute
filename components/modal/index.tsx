          ...
          <article>
            <div>
              ...
              <div className={Style.LevelInfo}>
                ...
                />
                <Level
                  heading={'Углублённый уровень'}
                  text={'Это уровень при котором даются начальные и углублённые знания по данной сфере, обычно это от 3х дисциплин и более'}
                  image={ProLevel}
                />
              </div>
            </div>
            <div>
              {spheresArray.map((elem, index) => (
                <Spheres
                  key={'sphere_' + titleProfile + '_' + index}
                  image={elem.image}
                  heading={elem.heading}
                  levelBase={elem.levelBase}
                />
              ))}
            </div>
          </article>
