  ...
  return (
    <article
      className={classNames(
        Style.Table,
        lotTable[array.length as keyof typeof lotTable],
        StyleGapOutside[gapOutside as keyof typeof StyleGapOutside],
      )}>
      {array.map((elem, index) => {
        if (array.length === 2 || array.length === 3 || array.length === 6) {
          return index % 2 === 0 ? (
            <Cell
              key={'table_' + index}
              numberOrImage={elem.numberOrImage}
              ...
              gapInside={gapInside}
              imgSize={imgSize}
              link={elem.link === undefined ? null : elem.link}
            />
          ) : (
            <Cell
              key={'table_' + index}
...
export function TableImages({ array, styleTableImages }: PropsTableImages) {
  return (
    <div
      className={classNames(
        Style.Table,
        styleTableImages === 'twoToOne'
          ...
          : styleTableImages === 'sixTable'
            ? Style.TableSix
            : StyleTableImages[array.length as keyof typeof StyleTableImages],
      )}>
      {array.map((elem, index) => (
        <CellImage
          positionUp={elem.position}
          key={'table_images_' + elem.heading}
          linkImage={elem.linkImage}
          heading={elem.heading}
          text={elem.text}
          twoToOne={styleTableImages === 'twoToOne'}
