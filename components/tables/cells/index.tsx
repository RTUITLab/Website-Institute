  ...
  return (
    <article
      className={classNames(
        Style.Cell,
        TableColor[background as keyof typeof TableColor],
        TableSide[side as keyof typeof TableSide],
        TableGap[gapInside as keyof typeof TableGap],
      )}>
      {typeof numberOrImage === 'number' || typeof numberOrImage === 'string' ? (
        <h1>{numberOrImage}</h1>
      ) : (
        <Image className={ImageSize[imgSize as keyof typeof ImageSize]} src={numberOrImage} alt={heading} />
      )}
      <div>
        <h2>{heading}</h2>
        <div>
          {text === null ? (
            ...
            text.map((elem, index) =>
              link !== null ? (
                <Link key={'link_' + heading + '_' + index} href={link[index] !== null ? link[index] : './'}>
                  {elem}
                </Link>
              ) : (
                <p key={heading + '_text_' + index}>{elem}</p>
              ),
            )
          )}
        </div>
...
export function CellTransition({
  ...
  return (
    <Link
      ...
      className={Style.CellTransition}
      style={{ gridArea: 'cell-' + gridAreaNumber }}
      href={linkPage}>
