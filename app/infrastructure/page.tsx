...
export default function Infrastructure()
{
    ...
    const textCampus = ["Рассказываем и показываем, основной кампус РТУ МИРЭА, на территории которого будет проходить 95% всех занятий студентов института информационных технологий.","Долгое время после своего создания в 1947 году вуз базировался на разных площадках. Собственный дом он начал обретать только в 1969 году, тогда началось строительство на тот момент современного учебного комплекса общей площадью 100 тыс. м2 на Юго-Западе Москвы.","В 1976 году первым введен в строй корпус Б. Здесь размещались ректорат и деканаты (ныне институт радиоэлектроники и информатики). Занятия всех факультетов в здании на проспекте Вернадского, 78 в корпусе А начались в 1979/1980 учебном году. Строительство здания МИРЭА завершилось в 80-е годы. Учебные площади институтов разместились в 6-ти корпусах.","Именно здесь сегодня расположено большинство мегалабораторий, аудиторий, внеучебной инфраструктуры и институт информационных технологий. В кампусе на проспекте Вернадского, 78 находится и легендарный коридор длиной 459 метров. В его центре — не менее легендарные «Штаны», лестницы на нижний этаж. Предшествующие поколения окрестили их так потому, что лестницы напоминают ноги гимнаста, который присаживается на шпагат."];
    ...
    const ImagesCampus = [
        Campus_photo_1, Campus_photo_2, Campus_photo_3, CampusUp
    ]
    ...
    const textLastCampus = ["Оставшиеся 5% учебных занятий проводятся в другом кампусе РТУ МИРЭА, который находится по адресу \"г. Москва, улица Малая Пираговка, д. 1, стр. 5\". В этом кампусе студенты изучают такие дисциплины, как \"Философия\", а также могут заниматься \"Правоведением\" и \"Искусственным интеллектом\". Однако занятия по последним двум дисциплинам также могут проводиться и в кампусе на Проспекте Вернадского, д. 78.", "В дополнение к этому, существуют ещё пять кампусов и отделений, где у студентов ИТ не проводятся академические занятия. Однако, в этих местах могут проходить различные внеучебные мероприятия.", "Например, на проспекте Вернадского, дом 86, кампус института тонких химических технологий  предоставляет возможность проведения разнообразных развлекательных студенческих мероприятий.", "Большой бассейн на улице Стромынка, дом 20, подходит для водных спортивных мероприятий.", "А при похождение обучения на военнкой кафере, студенты могут проходить обучение в строение, расположенном на улице Усачёва, дом 7/1."];
    const textHostel = ["Абитуриентам, проживающим за автодорогой А-108, и поступающим на направления бакалавриата и магистратуры нашего университета, предоставляется возможность получить общежитие в РТУ МИРЭА. Возможность получения места в общежитие также доступна во время учебы через включение в специальную очередь.", "На момент проведения приемной компании в 2023 году РТУ МИРЭА имеет в своём распоряжение шесть общежитий для студентов. Расстояние до основного кампуса может варьироваться от 10 минут до 1 часа в зависимости от выбранного общежития. Кроме того, университет имеет возможность размещения абитуриентов, в общежитиях своих партнеров."];
    const laboratory = ["На базе института информационных технологий были созданы четыре мегалаборатории, в которых осуществляется полноценное обучение студентов, проведение научных исследований, а также реализация производственных проектов, соответствующих специфике каждой лаборатории. Все мегалаборатории имеют свои уникальные особенности и оснащены ценным и передовым оборудованием, которое студенты имеют возможность использовать как во время обучения, так и при необходимости для реализации собственных исследовательских задач.", "Часть лабораторий создана при активном взаимодействии с различными партнёрами, среди которых компании \"ВКонтакте\" и корпорация \"Ростелеком\". Это сотрудничество позволяет не только обогатить учебный процесс богатым практическим опытом, но и создает условия для разработки инновационных проектов, направленных на решение актуальных задач в сфере информационных технологий.", "Каждая мегалаборатория имеет свою уникальную специализацию и направленность, обеспечивая студентам богатый и разносторонний опыт работы с передовым оборудованием и реальными проектами, что способствует их успешной подготовке к будущей профессиональной деятельности."];
    const textClassroomIIT = ["Институт информационных технологий располагает более чем 28 компьютерными учебными аудиториями и стандартными лабораториями, предназначенными для проведения семинаров и практических занятий с использованием передового, мощного и специализированного оборудования, идеально подходящего для охвата различных дисциплин. Эти помещения находятся в корпусе «Г», а также в локации института в корпусе «А».", "Компьютерное оборудование, предоставляемое в аудиториях, подходит как для решения базовых задач, так и для выполнения тяжелых и сложных заданий, таких как 3D-моделирование, анализ данных, создание моделей поведения объектов, проектирование логических схем, работа с нейронными сетями, а также работа с инженерным оборудованием в требовательных средах разработки. Это позволяет студентам эффективно осваивать программирование и технологии, подготавливая их к дальнейшей реализации сложных проектов."];
                ...
                <Section id={id[0].link}>
                    ...
                    <div className={Style.LastCampusAndHostel}>
                        <div>
                            ...
                            {textLastCampus.map((elem, index) => <p key={"last_campus_"+index}>{elem}</p>)}
                        </div>
